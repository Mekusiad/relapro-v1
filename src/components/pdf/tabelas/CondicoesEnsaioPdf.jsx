...
const CondicoesEnsaioPdf = ({ ensaio }) => {
  ...
  if (!ensaioData.temperatura && !ensaioData.umidade && !ensaioData.frequencia && !ensaio.dataEnsaio) {
    return null;
  }
    ...
    <View style={styles.container}>
      ...
      <View style={{ border: '1px solid #eeeeee', borderRadius: 3 }}>
        ...
        <View style={[styles.row, { borderTopWidth: 1 }]}>
          ...
          <Text style={styles.cellValue}>{ensaio.dataEnsaio ? formatDate(ensaio.dataEnsaio) : 'N/A'}</Text>
        </View>
        {/* RESPONSÁVEL (NOVO) */}
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.cellValue}>{ensaioData.temperatura || 'N/A'}</Text>
        </View>
        <View style={styles.row}>
