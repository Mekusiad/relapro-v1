...
function UnitSelectionModal({ isOpen, onRequestClose, instances, onSelectUnit }) {
    ...
    return (
        <AnimatedModal isOpen={isOpen} onRequestClose={onRequestClose}>
            <div className="unit-selection-modal">
                ...
                <p>Existem várias unidades deste equipamento. Por favor, selecione qual delas você deseja editar as informações.</p>
                
                <div className="units-list">
                    {instances.map((instance, index) => (
                        <div key={instance.id} className="unit-item">
                            <span className="unit-name">
                                {instance.nomeEquipamento || instance.nome} - Unidade {index + 1}
                                {instance.info?.numeroSerie && <span className="unit-serial"> (Série: {instance.info.numeroSerie})</span>}
                            </span>
                            <Button variant="secondary" onClick={() => handleSelect(instance)}>
