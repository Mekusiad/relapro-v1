...
function PendenciaFormModal({ isOpen, onRequestClose, onSave, pendenciaToEdit }) {
    ...
    return (
        <AnimatedModal isOpen={isOpen} onRequestClose={onRequestClose}>
            <div className="modal-header">
                <h2>{isEditMode ? 'Editar Faturamento Pendente' : 'Adicionar Faturamento Pendente'}</h2>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                    ...
                    <input name="motivo" value={formData.motivo || ''} onChange={handleChange} className="input" required />
                </div>
                <div className="form-row">
                    <div className="form-group">
                        <label>Valor (R$)</label>
                        <input type="number" step="0.01" name="valor" value={formData.valor || ''} onChange={handleChange} className="input" placeholder="2500.00" required />
                    </div>
                    <div className="form-group">
                        <label>Pedido de Compra</label>
                        <input name="pedidoCompra" value={formData.pedidoCompra || ''} onChange={handleChange} className="input" />
                    </div>
                </div>
                 <div className="form-group">
                ...
                <div className="form-actions">
                    ...
                    <Button type="submit" variant="primary">{isEditMode ? 'Salvar Alterações' : 'Salvar Pendência'}</Button>
                </div>
            </form>
        </AnimatedModal>
    );
